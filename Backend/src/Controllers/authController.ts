...
export const register = async(req:Request, res:Response)=>{
    const {nome, email, senha, perfil, telefone, cuidadorId} = req.body;
    ...
    try{
        ...
        const novoUsuario = await prisma.usuario.create({
            data:{
                ...
                perfil,
                telefone, 
                cuidadorId //pode ser null
            },
            select:{id: true, nome:true, perfil:true, telefone: true, cuidadorId: true, criadoEm:true}
        });
        //geração token caso faça automaticamente o login do usuário
